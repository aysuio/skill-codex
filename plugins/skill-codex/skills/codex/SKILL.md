...
## Defaults
- **Session lane**: `codex-5.6-sol-xhigh`
- **Model**: `gpt-5.6-sol`
...
This skill owns the `codex-5.6-sol-xhigh` session lane. Keep that lane separate from other Codex lanes, especially `codex-5.6-terra-xhigh` used by the `codex-terra` skill. If the user asks for both independent Codex reasoners, use `codex-dual`; that skill owns separate dual-lane sessions.
...
## Running a Task
1. **Bind one Codex session lane to one Claude session.** After each `codex exec` run, extract the `session id` from stderr and store it for the current Claude conversation under the `codex-5.6-sol-xhigh` lane only. On subsequent Sol Codex calls, always resume that exact stored session ID. Do **not** use `resume --last` or any global "most recent session" fallback — that can attach the wrong Codex session from another Claude conversation or another model lane. Only start a new Sol session if no `codex-5.6-sol-xhigh` session ID is stored, resume fails, or the user explicitly asks for a new Sol session.
...
## Following Up
- After every `codex` command, extract the session ID from stderr: `grep -a 'session id:' /tmp/codex-stderr.txt | sed 's/\x1b\[[0-9;]*m//g' | awk -F'session id: ' '{print $2}' | tr -d '[:space:]'`. Store this ID under `codex-5.6-sol-xhigh` for subsequent resume calls. **This is critical** — without the session ID, you cannot resume the correct lane.
