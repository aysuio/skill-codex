---
name: codex-dual
description: Use when the user asks Claude Code to run or resume two independent Codex reasoning sessions, compare GPT-5.6 Sol and Terra answers, run dual Codex review, consult both Codex lanes, or open/resume both GPT-5.6 Sol xhigh and GPT-5.6 Terra xhigh Codex sessions for the same task.
---

# Dual Codex Skill Guide

## Defaults
- **Session lane A**: `codex-dual-5.6-sol-xhigh`
- **Lane A model**: `gpt-5.6-sol`
- **Session lane B**: `codex-dual-5.6-terra-xhigh`
- **Lane B model**: `gpt-5.6-terra`
...
This skill owns the `codex-dual-5.6-sol-xhigh` and `codex-dual-5.6-terra-xhigh` session lanes. Keep both lanes separate from the single-lane `codex-5.6-sol-xhigh` and `codex-5.6-terra-xhigh` sessions.
...
3. When starting a **new Sol lane**, use `--sandbox danger-full-access` unless the user explicitly requests a different sandbox mode. Assemble the command with:
   - `-m, --model gpt-5.6-sol`
   ...
   - `2>/tmp/codex-sol-stderr.txt`
4. When starting a **new Terra lane**, use the same command shape with `-m gpt-5.6-terra`, `--config model_reasoning_effort="xhigh"`, and `2>/tmp/codex-terra-stderr.txt`.
...
5. When **resuming**, pass the new prompt as a **positional argument** after each lane's session ID:
   ```bash
   codex exec --skip-git-repo-check resume <CODEX_SOL_SESSION_ID> "your prompt here" 2>/tmp/codex-sol-stderr.txt
   codex exec --skip-git-repo-check resume <CODEX_TERRA_SESSION_ID> "your prompt here" 2>/tmp/codex-terra-stderr.txt
   ```
...
6. **IMPORTANT**: Codex outputs the session ID and metadata on **stderr**. Never use `2>/dev/null` — it swallows the session ID. Instead, redirect stderr to lane-specific temp files and extract the session IDs with:
   ```bash
   grep -a 'session id:' /tmp/codex-sol-stderr.txt | sed 's/\x1b\[[0-9;]*m//g' | awk -F'session id: ' '{print $2}' | tr -d '[:space:]'
   grep -a 'session id:' /tmp/codex-terra-stderr.txt | sed 's/\x1b\[[0-9;]*m//g' | awk -F'session id: ' '{print $2}' | tr -d '[:space:]'
   ```
   Store the Sol ID under `codex-dual-5.6-sol-xhigh` and the Terra ID under `codex-dual-5.6-terra-xhigh`. Only show the full stderr content if the user explicitly requests to see thinking tokens or if debugging is needed.
...
| --- | --- | --- |
| Read-only review or analysis | `read-only` | `--sandbox read-only 2>/tmp/codex-sol-stderr.txt` and `--sandbox read-only 2>/tmp/codex-terra-stderr.txt` |
...
## Following Up
- After every `codex-dual` command, extract both session IDs from stderr and store them under `codex-dual-5.6-sol-xhigh` and `codex-dual-5.6-terra-xhigh`. **This is critical** — without the session IDs, you cannot resume the correct dual lanes.
